...
```{r}
<<<<<<< HEAD
#dummy, coded 1 as below or equal to 35 (low SE) or 0 as higher than 35
#or 0 if below or equal to 45 etc.
self_esteem$dum_35_inp <- ifelse(self_esteem$rosenberg_inp <= 35, 1, 0)
self_esteem$dum_40_inp <- ifelse(self_esteem$rosenberg_inp <= 40, 1, 0)
self_esteem$dum_45_inp <- ifelse(self_esteem$rosenberg_inp <= 45, 1, 0)
=======
...
>>>>>>> 45b34071e78a41a50c38fbec03117e6b2d5dcb5e
...
##3.3 Add time variable for survival analysis
...
df_no_cov <- df_no_cov[order(df_no_cov$id, df_no_cov$year), ]

# Create the time variable
df_no_cov <- transform(df_no_cov, time = year - ave(year, id, FUN = min))
```

##3.4 Save data and clean
...
##4.1. Simple Tables
```{r}
table(df_no_cov$laid_off_na_corrected, df_no_cov$dum_35_inp)
table(df_no_cov$laid_off_na_corrected, df_no_cov$dum_40_inp)
table(df_no_cov$laid_off_na_corrected, df_no_cov$dum_45_inp)
...
ggplot(data = df_no_cov, aes(x = rosenberg_inp, y = laid_off_na_corrected)) +
...
surv_object <- Surv(time = df_no_cov$time, event = df_no_cov$laid_off_na_corrected)

km_fit <- survfit(surv_object ~ 1)  # '~ 1' indicates no stratification

ggsurvplot(km_fit, data = df_no_cov, conf.int = TRUE, 
...
```{r}
surv_object <- Surv(time = df_no_cov$time, event = df_no_cov$laid_off_na_corrected)
km_fit_stratified <- survfit(surv_object ~ dum_35_inp, data = df_no_cov)
ggsurvplot(km_fit_stratified, data = df_no_cov, conf.int = TRUE,
...
```{r}
surv_object <- Surv(time = df_no_cov$time, event = df_no_cov$laid_off_na_corrected)
km_fit_stratified <- survfit(surv_object ~ dum_40_inp, data = df_no_cov)
ggsurvplot(km_fit_stratified, data = df_no_cov, conf.int = TRUE,
           palette = c("blue", "red"),   # Optional: specify colors
           title = "Stratified Kaplan-Meier Survival Curves",
           xlab = "Time", ylab = "Survival Probability",
...
```{r}
surv_object <- Surv(time = df_no_cov$time, event = df_no_cov$laid_off_na_corrected)
km_fit_stratified <- survfit(surv_object ~ dum_45_inp, data = df_no_cov)
ggsurvplot(km_fit_stratified, data = df_no_cov, conf.int = TRUE,
           palette = c("blue", "red"),   # Optional: specify colors
           title = "Stratified Kaplan-Meier Survival Curves",
           xlab = "Time", ylab = "Survival Probability",
...
df_no_cov$time_interval <- cut(df_no_cov$time, breaks = seq(0, max(df_no_cov$time), by = 1), include.lowest = TRUE, right = FALSE)


m1_sub_35 <- coxph(Surv(time, laid_off_na_corrected) ~ dum_35_inp + strata(time_interval), data = df_no_cov)

summary(m1_sub_35) #interpretation: if m1_sub_35 is equal to 1, this corresponds to a 102% higher risk in getting fired compared to a higher self-esteem. This effect is statistically significantly different from 0. 


m1_sub_40 <- coxph(Surv(time, laid_off_na_corrected) ~ dum_40_inp + strata(time_interval), data = df_no_cov)

summary(m1_sub_40)

m1_sub_45 <- coxph(Surv(time, laid_off_na_corrected) ~ dum_45_inp + strata(time_interval), data = df_no_cov)

summary(m1_sub_45)

m1_cont <- coxph(Surv(time, laid_off_na_corrected) ~ rosenberg_inp + strata(time_interval), data = df_no_cov)
