...
both_available <- sum(!is.na(df_no_cov$rosenberg_inp) & !is.na(df_no_cov$laid_off))

# Count the number of observations where one of them is missing
one_missing <- sum(is.na(df_no_cov$rosenberg_inp) | is.na(df_no_cov$laid_off))

# Count the number of observations where rosenberg of them is missing
rosenberg_missing <- sum(is.na(df_no_cov$rosenberg_inp) & !is.na(df_no_cov$laid_off))

laid_off_missing <- sum(!is.na(df_no_cov$rosenberg_inp) & is.na(df_no_cov$laid_off))
...
- 41.000 people without Rosenberg_Inp, so people who have never participated in the personality survey 
...
```{r}
df_no_cov_nona <- subset(df_no_cov, !is.na(rosenberg_inp)&!is.na(laid_off))
```
...
Notably: proportion of lay-offs is much higher in those that have filled out the survey. 

Potential attrition problem (???)

```{r}

table(df_no_cov$laid_off[is.na(df_no_cov$rosenberg_inp) == T])
table(df_no_cov$laid_off[is.na(df_no_cov$rosenberg_inp) == F])
```
...
df_no_cov_nona <- df_no_cov_nona[order(df_no_cov_nona$id, df_no_cov_nona$year), ]

# Create the time variable
df_no_cov_nona <- transform(df_no_cov_nona, time = year - ave(year, id, FUN = min))
```
...
# Save the self_esteem dataframe as a CSV file
write.csv(df_no_cov_nona, file_path, row.names = FALSE)
...
#4. Gathering Control Variables

##4.1. From background survey
...
##4.2. Checking background control variables for outliers and creating dummies
```{r}
